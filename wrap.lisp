      ...
      (cffi:with-pointer-to-vector-data (buf buf)
        (let ((blocks-remaining required-blocks)
          ...
          (dotimes (sqe-chunk sqe-chunks)
            ...
            (let ((next-chunk-size (min osicat-posix::iov-max blocks-remaining)))
              (format t "Next chunk size: ~D~%" next-chunk-size)
              (dotimes (i next-chunk-size)
                (format t "Handling chunk: ~D~%" i)
                (let ((bytes-to-read (min bytes-remaining block-size))
                      (ptr (cffi:mem-aptr iovecs '(:struct osicat-posix::iovec) (+ chunks-processed i))))
                  ...
                  (cffi:incf-pointer iovecs-ptr)))
              (let ((sqe (io_uring_get_sqe ring)))
                (io_uring_prep_readv sqe fd iovecs-ptr required-blocks 0))))))
